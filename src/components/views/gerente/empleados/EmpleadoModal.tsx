  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[450px] p-0 gap-0 max-h-[90vh] overflow-y-auto">
        <EmpleadoForm
          key={`${open ? "open" : "closed"}-${empleadoParaEditar?.id ?? "new"}`}
          empleadoParaEditar={empleadoParaEditar ?? null}
...
function EmpleadoForm({
  ...
  const [username, setUsername] = useState(empleadoParaEditar?.username ?? "");
  const [nombre, setNombre] = useState(empleadoParaEditar?.nombre ?? "");
  const [apellidos, setApellidos] = useState(empleadoParaEditar?.apellidos ?? "");
  const [genero, setGenero] = useState<Genero | "">(empleadoParaEditar?.genero ?? "");
  const [fechaNacimiento, setFechaNacimiento] = useState(empleadoParaEditar?.fechaNacimiento ?? "");
  const [dni, setDni] = useState(empleadoParaEditar?.dni ?? "");
  const [esIndefinido, setEsIndefinido] = useState(
    empleadoParaEditar?.tipoContrato === TipoContrato.INDEFINIDO
  );
  const [finContrato, setFinContrato] = useState(empleadoParaEditar?.finContrato ?? "");
  const [rol, setRol] = useState<RolEmpleado | "">(empleadoParaEditar?.rol ?? "");
  const [imagenPerfil, setImagenPerfil] = useState(empleadoParaEditar?.imagenPerfil ?? "");
  const [imagenNombre, setImagenNombre] = useState(
    empleadoParaEditar?.imagenPerfil?.split("/").pop() ?? ""
  );
